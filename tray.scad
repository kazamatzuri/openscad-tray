 ...
 *
 * Multi-tray with unequal number of rows per column, equally distributed: first column has 4 rows, second column has 2
 * rows and final column as 3 rows. tray([100, 60, 30], n_columns=3, n_rows=[4,2,3]);
 *
 ...
 * tray([100, 60, 30], n_rows=3, n_columns=[4,3,2], rows=[0.3, 0.7], columns=[false, [0.25, 0.5], false],
 * rows_first=true);
 *
 */
            ...
            rows_first = false, magnets = false, magnet_height = undef, magnet_radius = undef)
{
    ...
    if (magnets == true)
    {
        ...
        difference()
        {
            tray_module(dimensions = dimensions, n_columns = n_columns, n_rows = n_rows, columns = columns, rows = rows,
                        thickness = thickness, curved = curved, dividers_height = dividers_height,
                        dividers_thickness = dividers_thickness, bottom_bevel_radius = bottom_bevel_radius,
                        top_bevel_radius = top_bevel_radius,
                        dividers_bottom_bevel_radius = dividers_bottom_bevel_radius,
                        dividers_top_bevel_radius = dividers_top_bevel_radius, bottom_thickness = bottom_thickness,
                        rows_first = rows_first);
            translate([ magnet_shift, magnet_shift, 0 ])
            {
                cube([ 2 * magnet_radius + 4, 2 * magnet_radius + 4, box_height ]);
            }
            translate([ dimensions[0] - magnet_shift - (2 * magnet_radius + 4), magnet_shift, 0 ])
            {
                cube([ 2 * magnet_radius + 4, 2 * magnet_radius + 4, box_height ]);
            }
            translate([ magnet_shift, dimensions[1] - magnet_shift - (2 * magnet_radius + 4), 0 ])
            {
                cube([ 2 * magnet_radius + 4, 2 * magnet_radius + 4, box_height ]);
            }
            translate([
                dimensions[0] - magnet_shift - (2 * magnet_radius + 4),
                dimensions[1] - magnet_shift - (2 * magnet_radius + 4), 0
            ])
            {
                cube([ 2 * magnet_radius + 4, 2 * magnet_radius + 4, box_height ]);
            }
    ...
    else
    {
        tray_module(dimensions = dimensions, n_columns = n_columns, n_rows = n_rows, columns = columns, rows = rows,
                    thickness = thickness, curved = curved, dividers_height = dividers_height,
                    dividers_thickness = dividers_thickness, bottom_bevel_radius = bottom_bevel_radius,
                    top_bevel_radius = top_bevel_radius, dividers_bottom_bevel_radius = dividers_bottom_bevel_radius,
                    dividers_top_bevel_radius = dividers_top_bevel_radius, bottom_thickness = bottom_thickness,
                    rows_first = rows_first);
    }
...
module tray_module(dimensions, n_columns, n_rows, columns, rows, thickness, curved, dividers_height, dividers_thickness,
                   bottom_bevel_radius, top_bevel_radius, dividers_bottom_bevel_radius, dividers_top_bevel_radius,
                   bottom_thickness, rows_first)
{
    ...
    if (rows_first == true)
    {
        translate([ ext_width, 0, 0 ]) rotate([ 0, 0, 90 ])
            tray([ ext_length, ext_width, ext_height ], n_columns = n_rows, n_rows = n_columns, columns = rows,
                 rows = columns, thickness = thickness, curved = curved, dividers_height = dividers_height,
                 dividers_thickness = dividers_thickness, bottom_bevel_radius = bottom_bevel_radius,
                 top_bevel_radius = top_bevel_radius, dividers_bottom_bevel_radius = dividers_bottom_bevel_radius,
    ...
    else
    {
        ...
        cell_bottom_bevel_radius =
            dividers_bottom_bevel_radius != undef ? dividers_bottom_bevel_radius : bottom_bevel_radius;
        ...
        all_cells_length =
            [for (i = [0:n_columns - 1])(ext_length - 2 * thickness_diff) + cell_thickness * (column_n_rows[i] - 1)];
        ...
        column_width = [for (i = [0:n_columns - 1])(columns == false ? (i + 1) * cell_width
                                                                     : (i != 0 && i == n_columns - 1 ? 1 : columns[i]) *
                                                                           all_cells_width)];
        // Create list of rows.
        row_length = [for (i = [0:n_columns - 1])[for (j = [0:column_n_rows[i] - 1])(
            rows == false || rows[i] == false
                ? (j + 1) * cell_length[i]
                : (j != 0 && j == column_n_rows[i] - 1 ? 1 : rows[i][j]) * all_cells_length[i])]];

        // Draw all cells.
        difference()
        {
            translate([ thickness_diff, thickness_diff, thickness_diff ]) for (i = [0:n_columns - 1])
            {
                for (j = [0:column_n_rows[i] - 1])
                {
                    translate([
                        i == 0 ? 0 : column_width[i - 1] - i * cell_thickness,
                        j == 0 ? 0 : row_length[i][j - 1] - j * cell_thickness, 0
                    ])
                        tray_single(
                            [
                                column_width[i] - (i == 0 ? 0 : column_width[i - 1]),
                                row_length[i][j] - (j == 0 ? 0 : row_length[i][j - 1]),
                                cell_height
                            ],
                            thickness = cell_thickness, curved = curved, bottom_thickness = bottom_thickness,
                            bottom_bevel_radius = cell_bottom_bevel_radius, top_bevel_radius = cell_top_bevel_radius);
                }
            }

            // Scoop out top if dividers_height is provided.
            if (dividers_height != undef)
                tray_scoop(dimensions, dividers_height, thickness = thickness, curved = curved,
                           top_bevel_radius = top_bevel_radius)
                    _tray_rounded_cube([ int_width, int_length, 2 * (ext_height - dividers_height) ],
                                       r = (top_bevel_radius >= 0 ? top_bevel_radius : (curved ? thickness : 0)),
...
{
    tray_single(dimensions, thickness = thickness, curved = curved, bottom_bevel_radius = 0,
                top_bevel_radius = top_bevel_radius, bottom_thickness = bottom_thickness);
}

// Draws an inverted rounded cube used to scoop out inside using difference().
module tray_scoop(dimensions, dividers_height, thickness = 2, curved = true, dividers_thickness = undef,
                  top_bevel_radius = undef)
{
    ...
    int_width = ext_width - 2 * thickness;
    int_length = ext_length - 2 * thickness;
    translate([ thickness, thickness, dividers_height ]) _tray_rounded_cube(
        [ int_width, int_length, 2 * (ext_height - dividers_height) ],
        r = (top_bevel_radius != undef ? top_bevel_radius : (curved ? thickness : 0)), x = true, y = true, z = true);
}

// Draws a single tray.
module tray_single(dimensions, thickness = 2, curved = true, bottom_thickness = undef, bottom_bevel_radius = undef,
                   top_bevel_radius = undef)
{
    ...
    difference()
    {
        _tray_rounded_cube([ ext_width, ext_length, ext_height ], r = ext_top_bevel_radius, x = true,
                           rx = [ 0, ext_top_bevel_radius, ext_top_bevel_radius, 0 ], y = true,
                           ry = [ ext_top_bevel_radius, ext_top_bevel_radius, 0, 0 ]);
        if (_bottom_thickness > 0)
            translate([ thickness, thickness, _bottom_thickness ])
                _tray_rounded_cube([ int_width, int_length, ext_height ], r = int_top_bevel_radius, x = true,
                                   rx = [ int_bottom_bevel_radius, 0, 0, int_bottom_bevel_radius ], y = true,
                                   ry = [ 0, 0, int_bottom_bevel_radius, int_bottom_bevel_radius ]);

        else
            translate([ thickness, thickness, 0 ]) _tray_rounded_cube([ int_width, int_length, ext_height ],
...
/*
    ...
    Arguments:
        ...
        - xcorners = Array of 4 booleans, one for each X side of the cube, if true then round that side. Default: [true,
   true, true, true]
        - ycorners = Array of 4 booleans, one for each Y side of the cube, if true then round that side. Default: [true,
   true, true, true]
        - zcorners = Array of 4 booleans, one for each Z side of the cube, if true then round that side. Default: [true,
   true, true, true]
...
module _tray_rounded_cube(dim, r = 1, x = false, y = false, z = true, xcorners = [ true, true, true, true ],
                          ycorners = [ true, true, true, true ], zcorners = [ true, true, true, true ], center = false,
                          rx = [ undef, undef, undef, undef ], ry = [ undef, undef, undef, undef ],
                          rz = [ undef, undef, undef, undef ], $fn = 128)
{
    translate([
        (center == true ? (-(dim[0] / 2)) : 0), (center == true ? (-(dim[1] / 2)) : 0),
        (center == true ? (-(dim[2] / 2)) : 0)
    ])
    {
        difference()
        {
            ...
            if (z)
            {
                ...
                {
                    if (zcorners[0])
                        translate([ 0, dim[1] - (rz[0] == undef ? r : rz[0]) ])
                        {
                            _tray_rotate_around(
                                [ 0, 0, 90 ], [ (rz[0] == undef ? r : rz[0]) / 2, (rz[0] == undef ? r : rz[0]) / 2, 0 ])
                            {
                                _tray_meniscus(h = dim[2], r = (rz[0] == undef ? r : rz[0]), fn = $fn);
                            }
                        }
                    if (zcorners[1])
                        translate([ dim[0] - (rz[1] == undef ? r : rz[1]), dim[1] - (rz[1] == undef ? r : rz[1]) ])
                        {
                            _tray_meniscus(h = dim[2], r = (rz[1] == undef ? r : rz[1]), fn = $fn);
                        }
                    if (zcorners[2])
                        translate([ dim[0] - (rz[2] == undef ? r : rz[2]), 0 ])
                        {
                            _tray_rotate_around(
                                [ 0, 0, -90 ],
                                [ (rz[2] == undef ? r : rz[2]) / 2, (rz[2] == undef ? r : rz[2]) / 2, 0 ])
                            {
                                _tray_meniscus(h = dim[2], r = (rz[2] == undef ? r : rz[2]), fn = $fn);
                            }
                        }
                    if (zcorners[3])
                        _tray_rotate_around([ 0, 0, -180 ],
                                            [ (rz[3] == undef ? r : rz[3]) / 2, (rz[3] == undef ? r : rz[3]) / 2, 0 ])
                        {
                            _tray_meniscus(h = dim[2], r = (rz[3] == undef ? r : rz[3]), fn = $fn);
                        }
                }
            }
            ...
            if (y)
            {
                ...
                {
                    if (ycorners[0])
                        translate([ 0, 0, dim[2] - (ry[0] == undef ? r : ry[0]) ])
                        {
                            _tray_rotate_around(
                                [ 0, 180, 0 ],
                                [ (ry[0] == undef ? r : ry[0]) / 2, 0, (ry[0] == undef ? r : ry[0]) / 2 ])
                            {
                                _tray_rotate_around(
                                    [ -90, 0, 0 ],
                                    [ 0, (ry[0] == undef ? r : ry[0]) / 2, (ry[0] == undef ? r : ry[0]) / 2 ])
                                {
                                    _tray_meniscus(h = dim[1], r = (ry[0] == undef ? r : ry[0]));
                                }
                            }
                        }
                    if (ycorners[1])
                        translate([ dim[0] - (ry[1] == undef ? r : ry[1]), 0, dim[2] - (ry[1] == undef ? r : ry[1]) ])
                        {
                            _tray_rotate_around(
                                [ 0, -90, 0 ],
                                [ (ry[1] == undef ? r : ry[1]) / 2, 0, (ry[1] == undef ? r : ry[1]) / 2 ])
                            {
                                _tray_rotate_around(
                                    [ -90, 0, 0 ],
                                    [ 0, (ry[1] == undef ? r : ry[1]) / 2, (ry[1] == undef ? r : ry[1]) / 2 ])
                                {
                                    _tray_meniscus(h = dim[1], r = (ry[1] == undef ? r : ry[1]));
                                }
                            }
                        }
                    if (ycorners[2])
                        translate([ dim[0] - (ry[2] == undef ? r : ry[2]), 0 ])
                        {
                            _tray_rotate_around(
                                [ -90, 0, 0 ],
                                [ 0, (ry[2] == undef ? r : ry[2]) / 2, (ry[2] == undef ? r : ry[2]) / 2 ])
                            {
                                _tray_meniscus(h = dim[1], r = (ry[2] == undef ? r : ry[2]));
                            }
                        }
                    if (ycorners[3])
                        _tray_rotate_around([ 0, 90, 0 ],
                                            [ (ry[3] == undef ? r : ry[3]) / 2, 0, (ry[3] == undef ? r : ry[3]) / 2 ])
                        {
                            _tray_rotate_around(
                                [ -90, 0, 0 ],
                                [ 0, (ry[3] == undef ? r : ry[3]) / 2, (ry[3] == undef ? r : ry[3]) / 2 ])
                            {
                                _tray_meniscus(h = dim[1], r = (ry[3] == undef ? r : ry[3]));
                            }
                        }
                }
            }
            ...
            if (x)
            {
                ...
                {
                    if (xcorners[0])
                        translate([ 0, dim[1] - (rx[0] == undef ? r : rx[0]) ])
                        {
                            _tray_rotate_around(
                                [ 0, 90, 0 ], [ (rx[0] == undef ? r : rx[0]) / 2, 0, (rx[0] == undef ? r : rx[0]) / 2 ])
                            {
                                _tray_meniscus(h = dim[0], r = (rx[0] == undef ? r : rx[0]));
                            }
                        }
                    if (xcorners[1])
                        translate([ 0, dim[1] - (rx[1] == undef ? r : rx[1]), dim[2] - (rx[1] == undef ? r : rx[1]) ])
                        {
                            _tray_rotate_around(
                                [ 90, 0, 0 ], [ 0, (rx[1] == undef ? r : rx[1]) / 2, (rx[1] == undef ? r : rx[1]) / 2 ])
                            {
                                _tray_rotate_around(
                                    [ 0, 90, 0 ],
                                    [ (rx[1] == undef ? r : rx[1]) / 2, 0, (rx[1] == undef ? r : rx[1]) / 2 ])
                                {
                                    _tray_meniscus(h = dim[0], r = (rx[1] == undef ? r : rx[1]));
                                }
                            }
                        }
                    if (xcorners[2])
                        translate([ 0, 0, dim[2] - (rx[2] == undef ? r : rx[2]) ])
                        {
                            _tray_rotate_around(
                                [ 180, 0, 0 ],
                                [ 0, (rx[2] == undef ? r : rx[2]) / 2, (rx[2] == undef ? r : rx[2]) / 2 ])
                            {
                                _tray_rotate_around(
                                    [ 0, 90, 0 ],
                                    [ (rx[2] == undef ? r : rx[2]) / 2, 0, (rx[2] == undef ? r : rx[2]) / 2 ])
                                {
                                    _tray_meniscus(h = dim[0], r = (rx[2] == undef ? r : rx[2]));
                                }
                            }
                        }
                    if (xcorners[3])
                        _tray_rotate_around([ -90, 0, 0 ],
                                            [ 0, (rx[3] == undef ? r : rx[3]) / 2, (rx[3] == undef ? r : rx[3]) / 2 ])
                        {
                            _tray_rotate_around(
                                [ 0, 90, 0 ], [ (rx[3] == undef ? r : rx[3]) / 2, 0, (rx[3] == undef ? r : rx[3]) / 2 ])
                            {
                                _tray_meniscus(h = dim[0], r = (rx[3] == undef ? r : rx[3]));
                            }
                        }
                }
            }
...
module _tray_meniscus(h, r, fn = 128)
{
    ...
    difference()
    {
        ...
        translate([ 0, 0, -0.1 ])
        {
            cylinder(h = h + 0.4, r = r);
        }
    }
}

module _tray_rotate_around(a, v)
{
    translate(v)
    {
        rotate(a)
